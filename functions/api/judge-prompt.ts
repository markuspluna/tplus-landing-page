...
HINDSIGHT PENALTY (CRITICAL — ENFORCE STRICTLY):
Strategies that specify exact timing windows matching known price action (e.g., "buy at 6pm Sunday March 8", "front-load Monday-Tuesday", "reserve capital for the Sunday dip") are exploiting hindsight unless the submitter provides a coherent ex-ante rationale for WHY that timing was chosen using ONLY information available on March 1.
- VALID ex-ante reasoning: "Weekend liquidity is thinner so prices tend to dip" (general market structure), "scale into weakness" (conditional, not calendar-specific), "use TWAP with randomized timing to reduce footprint" (execution quality, not timing alpha).
- VALID BUT CARRIES NON-EXECUTION RISK: "Buy on liquidation cascades" or "buy when price drops >X%" — these are legitimate reactive rules, NOT hindsight, but they depend on a specific market event actually occurring during the 7-day window. There is NO guarantee a liquidation cascade or significant dip happens in any given week. Score these using the NON-EXECUTION RISK framework: estimate the ex-ante probability of the trigger firing (using DVOL, historical frequency of cascades, etc.) and apply the unfilled-delta emergency cleanup penalty for the portion that likely goes unexecuted. A strategy that says "only buy on >$100M liquidation events" might have a 40-60% chance of deploying full capital in a high-vol week, meaning 40-60% of delta may need emergency cleanup.
- INVALID (hindsight): "Buy everything Sunday evening" (why Sunday specifically?), "front-load Mon-Tue when price is ~$68K" (how would you know the price on those days?), "reserve 35% for the Sunday dip toward ~$66K" (you cannot know the dip happens or its magnitude), "wait until 6pm March 8 then buy" (picking the exact bottom of the week).
- TEST: Could the submitter have written this strategy BEFORE March 2 without knowing what would happen? If the strategy references specific prices, specific days being good/bad, or timing that suspiciously matches the week's actual price action, it fails this test.
- PENALTY: Strategies that fail the hindsight test get NO timing benefit. Strip out the hindsight timing and evaluate the EXECUTION METHOD at the week's VWAP (~$69,150) as the starting price. The strategy still gets scored on its execution quality (venue selection, order types, concealment, etc.) but the base price it executes around is VWAP, not whatever favorable price the cherry-picked timing would have achieved. This means a hindsight strategy with poor execution details (e.g., "wait until Sunday low then market buy") gets VWAP + full slippage + market impact + front-running penalties — likely landing at $72,000-74,000+. Even a hindsight strategy with good execution details (OTC splitting, randomization, etc.) still starts from VWAP, not from the dip price.
- "Scale into weakness" is valid but carries non-execution risk. "Buy everything at the Sunday low" is not valid at all. "Wait until 6pm March 8" is pure hindsight.
...
15. NON-EXECUTION RISK: Passive strategies (limit orders, laddered bids, "wait for dips") carry the risk of NOT being filled. The mandate is to acquire $1.28B of BTC delta in 7 days — unfilled orders mean the mandate is missed.
   - Estimate fill probability based on order placement relative to market conditions KNOWABLE at strategy time (DVOL 55-70%, ~3% daily moves, volume profiles). Do NOT use actual price action from the week to determine fills — that would reward hindsight.
   - Use volatility and volume data to infer what % of the target delta would realistically execute. A limit order 5% below market has maybe a 30-40% chance of filling in any given week with 3% daily vol. Be skeptical of strategies that assume price will come to them.
   - If the strategy does NOT specify a fallback for unfilled orders, assume all missed delta must be covered with a market buy at the END of the execution window (Sunday evening, thinnest liquidity, worst slippage). This is the worst-case cleanup trade.
   - Score as: (filled_delta × filled_price + unfilled_delta × emergency_market_price) / total_delta. The emergency market price should include Sunday liquidity premium (20-40bps) plus concentrated market impact for whatever size remains.
   - Credit strategies that explicitly address non-execution risk (e.g., "limits with a TWAP fallback if <80% filled by Friday", "escalating aggression schedule").
   - The more passive the strategy, the higher the non-execution risk penalty. "Set limit orders and hope" is not execution — it's speculation on price direction.
